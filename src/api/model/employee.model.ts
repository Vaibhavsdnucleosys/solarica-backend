...
export const createEmployeeModel = async (
  ...
  name: string, 
  phone?: string,
  company?: string // <--- Added parameter
) => {
  const existingUser = await prisma.user.findUnique({ where: { email } });
  if (existingUser) throw new Error("User with this email already exists");

  const validRoles = ['Sales', 'Accounting', 'Operation', 'Operation Employee', 'admin'];
  if (!validRoles.includes(roleName)) throw new Error("Role not allowed");
  ...
  const result = await prisma.$transaction(async (tx: any) => {
    // 1. Create User with Company
    const user = await tx.user.create({
      data: {
        name,
        email,
        phone,
        company, // <--- Save to User
        password: hashedPassword,
        roleId: role!.id
      },
      include: { role: true }
    ...
    // 2. Create Worker with same Company
    const worker = await tx.worker.create({
      data: {
        name,
        email,
        company, // <--- Save to Worker for consistency
        userId: user.id
      }
    });
...
// Update this to select the company field
export const getAllEmployeesModel = async () => {
  const users = await prisma.user.findMany({
    where: {
      isActive: true,
    ...
    select: {
      ...
      email: true,
      phone: true,
      company: true, // <--- Ensure this is selected
  ...
  return users.map((user: any) => ({
    ...user,
    category: user.role?.name || 'General' // category = department
  }));
};
// Update getAllEmployeesModel to return company
// export const getAllEmployeesModel = async () => {
//   const users = await prisma.user.findMany({
//     where: {
//       isActive: true,
...
//       email: true,
//       phone: true,
//       company: true, // Added company to selection
//       role: { select: { name: true } },
//       createdAt: true
//     }
//   });
